...
**Input**: [docs/spec-branch-reorganization-plan-2026-05-13.md](spec-branch-reorganization-plan-2026-05-13.md), existing Spec Kit artifacts under [specs/015-visual-refit](../specs/015-visual-refit), [specs/016-intent-set-migration](../specs/016-intent-set-migration), and [specs/017-capture-feedback-actions](../specs/017-capture-feedback-actions)
...
- [x] T011 Confirm `/Users/richelgomez/dev/capsule-app-spec-016` is clean before reconciliation using `git -C /Users/richelgomez/dev/capsule-app-spec-016 status --short` and record the result in [specs/016-intent-set-migration/tasks.md](../specs/016-intent-set-migration/tasks.md).
- [x] T012 [P] Compare local `016-intent-set-migration` against `origin/main` from `/Users/richelgomez/dev/capsule-app-spec-016` using `git diff --stat origin/main...HEAD` and record the summary in [specs/016-intent-set-migration/tasks.md](../specs/016-intent-set-migration/tasks.md).
...
- [x] T014 Run the Android gate in `/Users/richelgomez/dev/capsule-app-spec-016`: `ANDROID_HOME="$HOME/Library/Android/sdk" JAVA_HOME='/Applications/Android Studio.app/Contents/jbr/Contents/Home' ./gradlew :app:compileDebugKotlin :app:testDebugUnitTest :app:compileDebugAndroidTestKotlin :app:lintDebug`, then record results in [specs/016-intent-set-migration/tasks.md](../specs/016-intent-set-migration/tasks.md).
- [x] T015 Run the Supabase classifier gate in `/Users/richelgomez/dev/capsule-app-spec-016/supabase/functions/llm_gateway`: `npm run typecheck` and `npm run test:unit`, then record results in [specs/016-intent-set-migration/tasks.md](../specs/016-intent-set-migration/tasks.md).
- [x] T016 Run stale-label search in `/Users/richelgomez/dev/capsule-app-spec-016`: `rg -n "REMIND_ME|INSPIRATION|intent-set rename" app/src/main supabase/functions/llm_gateway`, then record expected no-implementation-hit results in [specs/016-intent-set-migration/quickstart.md](../specs/016-intent-set-migration/quickstart.md).
- [x] T017 Run intent-label coverage search in `/Users/richelgomez/dev/capsule-app-spec-016`: `rg -n "Intent\.READ_LATER|READ_LATER" app/src/main supabase/functions/llm_gateway`, then record intended Android and cloud classifier coverage in [specs/016-intent-set-migration/quickstart.md](../specs/016-intent-set-migration/quickstart.md).
- [x] T018 Confirm no ContactRef entity, Room migration, exported schema, or historical intent rewrite leaked into `/Users/richelgomez/dev/capsule-app-spec-016`; record the confirmation in [specs/016-intent-set-migration/tasks.md](../specs/016-intent-set-migration/tasks.md).
...
Commit/PR result: committed the staged `017` closeout work as `b872e38`, pushed `origin/017-capture-feedback-actions`, and opened PR #20. Self-review fixes were then committed as `1d0d100` and pushed to the same PR, adding database-backed active duplicate keys, exact-text active-key indexing, concurrent duplicate coverage, v5-to-v7 migration coverage, and androidTest schema assets. PR body includes S24 and Tab S9 physical QA evidence from [specs/017-capture-feedback-actions/tasks.md](../specs/017-capture-feedback-actions/tasks.md). Landing remains deferred pending review and the `016` dependency state.
...
- [x] T021 Confirm T020 is complete or explicitly deferred before mutating `/Users/richelgomez/dev/capsule-app-spec-017`; record the dependency state in [specs/017-capture-feedback-actions/tasks.md](../specs/017-capture-feedback-actions/tasks.md).
...
- [x] T024 Confirm the staged `017` files belong to duplicate capture, `Already saved`, note persistence, Room migration, or URL-hash dedupe contract work by comparing them to [specs/017-capture-feedback-actions/data-model.md](../specs/017-capture-feedback-actions/data-model.md).
...
- [x] T027 Verify Room schema version, exported schema JSON, and migration naming in `/Users/richelgomez/dev/capsule-app-spec-017/app/schemas/com.capsule.app.data.OrbitDatabase` against [specs/017-capture-feedback-actions/data-model.md](../specs/017-capture-feedback-actions/data-model.md).
- [x] T028 Run the Android gate in `/Users/richelgomez/dev/capsule-app-spec-017`: `ANDROID_HOME="$HOME/Library/Android/sdk" JAVA_HOME='/Applications/Android Studio.app/Contents/jbr/Contents/Home' ./gradlew :app:compileDebugKotlin :app:testDebugUnitTest :app:compileDebugAndroidTestKotlin :app:lintDebug`, then record results in [specs/017-capture-feedback-actions/tasks.md](../specs/017-capture-feedback-actions/tasks.md).
- [x] T029 Run focused duplicate-capture tests in `/Users/richelgomez/dev/capsule-app-spec-017` for URL duplicate, exact-text duplicate, deleted/archived duplicate exclusions, note persistence, reclassify, open-existing, and URL hydration reuse; record command names and results in [specs/017-capture-feedback-actions/tasks.md](../specs/017-capture-feedback-actions/tasks.md).
- [x] T030 Confirm duplicate audit metadata in `/Users/richelgomez/dev/capsule-app-spec-017` includes `existingEnvelopeId` and `matchedBy` but not raw text or full URLs; record confirmation in [specs/017-capture-feedback-actions/tasks.md](../specs/017-capture-feedback-actions/tasks.md).
...
- Deleted stale density launcher WebPs under `app/src/main/res/mipmap-*/`.
- Updated [docs/capture-overlay-followups.md](capture-overlay-followups.md) and [specs/015-visual-refit/tasks.md](../specs/015-visual-refit/tasks.md).
...
- [x] T033 Confirm T020 and T032 are complete or explicitly deferred before mutating `/Users/richelgomez/dev/capsule-app-015-phase1-split`; record the dependency state in [specs/015-visual-refit/tasks.md](../specs/015-visual-refit/tasks.md).
...
- [x] T036 Confirm staged `015` files belong to visual refit, overlay touch-bounds presentation, service health presentation, launcher/notification mark assets, docs, or [specs/015-visual-refit/tasks.md](../specs/015-visual-refit/tasks.md); reroute non-visual behavior changes to the owning branch before committing.
- [x] T037 Decide whether `T015-501` through `T015-506` in [specs/015-visual-refit/tasks.md](../specs/015-visual-refit/tasks.md) block the PR or become follow-up overlay customization work.
- [x] T038 Decide whether `T015-901`, `T015-904`, and `T015-905` in [specs/015-visual-refit/tasks.md](../specs/015-visual-refit/tasks.md) block the PR or become flag-flip follow-ups.
...
- [x] T040 Run the Android gate in `/Users/richelgomez/dev/capsule-app-015-phase1-split`: `ANDROID_HOME="$HOME/Library/Android/sdk" JAVA_HOME='/Applications/Android Studio.app/Contents/jbr/Contents/Home' ./gradlew :app:compileDebugKotlin :app:testDebugUnitTest :app:compileDebugAndroidTestKotlin :app:lintDebug`, then record results in [specs/015-visual-refit/tasks.md](../specs/015-visual-refit/tasks.md).
- [x] T041 Run the build-logic lint gate in `/Users/richelgomez/dev/capsule-app-015-phase1-split`: `./gradlew :build-logic:lint:test`, then record results in [specs/015-visual-refit/tasks.md](../specs/015-visual-refit/tasks.md).
- [x] T042 Record the known connected Compose harness limitation for flag-off instrumented tests in [specs/015-visual-refit/tasks.md](../specs/015-visual-refit/tasks.md) without marking physical/manual evidence as complete unless it was actually collected.
- [x] T043 Capture or explicitly defer flag-on manual screenshots for Diary, Settings, Capture sheet, post-capture pills, and bubble drag/remove in [specs/015-visual-refit/tasks.md](../specs/015-visual-refit/tasks.md).
